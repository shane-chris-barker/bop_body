...
class TestAngryHandler(unittest.TestCase):
    def setUp(self):
        self.mock_screen_updater = MagicMock()
        self.handler = AngryHandler(screen_updater=self.mock_screen_updater)
        self.dto = EventDTO(event_type=EventType.BOP_ANGRY)

    def test_handle_calls_display_text(self):
        self.handler.handle(self.dto)
        self.mock_screen_updater.display_text.assert_called_once_with("Bop is ANGRY!!!!!")